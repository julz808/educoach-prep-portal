-- =============================================================================
-- RUN DEPLOYMENT AND VERIFY SCRIPT
...
-- Check sub-skills were created
SELECT ts.product_type, ts.section_name, COUNT(ss.id) as sub_skill_count
FROM test_sections ts
LEFT JOIN sub_skills ss ON ss.section_id = ts.id
GROUP BY ts.product_type, ts.section_name, ts.section_order
ORDER BY ts.product_type, ts.section_order;
...
-- Additional verification: Check table existence and row counts
SELECT 
  'test_sections' as table_name, 
...
FROM pg_tables 
WHERE tablename IN ('test_sections', 'sub_skills', 'test_templates', 'user_progress', 'user_question_responses', 'user_sub_skill_performance')
ORDER BY tablename; 