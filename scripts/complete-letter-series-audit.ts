...
function calculateNext(positions: number[]): string | null {
  ...
  // Check incrementing by 2 each time (+2, +4, +6, +8...)
...
async function main() {
  ...
  console.log(`Auditing ${questions.length} Letter Series questions...\n`);
  ...
  for (const q of questions) {
    ...
    // Find what the stored answer maps to
    let storedValue = '';
    ...
    if (!isCorrect) {
      ...
      // Check if calculated answer is in options
      let foundInOptions = false;
      q.answer_options.forEach((opt: string, idx: number) => {
        if (extractLetters(opt) === calculated) {
          foundInOptions = true;
          console.log(`   ✓ Calculated answer IS in options (option ${String.fromCharCode(65 + idx)})`);
        }
      });

      if (!foundInOptions) {
        console.log(`   ⚠️  CRITICAL: Calculated answer NOT in options - question needs regeneration`);
      }
      console.log('');
    } else {
      console.log(`✅ ${q.test_mode} Q${q.question_order}: ${letters.join(' ')} → ${storedValue}`);
    }
  ...
  console.log('\nResults saved to /tmp/letter_series_complete_audit.json');
}
