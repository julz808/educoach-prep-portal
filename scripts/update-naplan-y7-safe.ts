/**
 * SAFE script to update NAPLAN Year 7 curriculum data from 6 difficulty levels to 3
...
const filePath = join(process.cwd(), 'src/data/curriculumData_v2/naplan-year7.ts');

console.log('Reading NAPLAN Year 7 file...\n');
...
for (let i = 0; i < lines.length; i++) {
  ...
  // Update header comment - change from 6 levels to 3
  if (line.includes('Difficulty Levels: 1-6')) {
    newLines.push(line.replace('Difficulty Levels: 1-6 (based on sample question tags)',
      'Difficulty Levels: 1-3 (Easy, Medium, Hard) - mapped from original NAPLAN levels'));
    newLines.push(' *');
    newLines.push(' * Difficulty Mapping:');
    newLines.push(' *   - Level 1 (Easy): Covers original NAPLAN levels 1-2');
    newLines.push(' *   - Level 2 (Medium): Covers original NAPLAN levels 3-4');
    newLines.push(' *   - Level 3 (Hard): Covers original NAPLAN levels 5-6');
    continue;
  }
  ...
  if (line.trim().startsWith('difficulty: 1,')) {
    line = line.replace('difficulty: 1,', 'difficulty: 1,  // Easy (original level 1)');
  }
  ...
  if (line.trim().startsWith('difficulty: 6,')) {
    line = line.replace('difficulty: 6,', 'difficulty: 3,  // Hard (mapped from old level 6)');
  }
...
console.log('✅ Successfully updated naplan-year7.ts!');
console.log('\nChanges made:');
console.log('  ✓ Updated header comments from 6 to 3 levels');
console.log('  ✓ Changed all difficulty_range from [1,2,3,4,5,6] to [1,2,3]');
console.log('  ✓ Remapped example difficulties (1-2→1, 3-4→2, 5-6→3)');
