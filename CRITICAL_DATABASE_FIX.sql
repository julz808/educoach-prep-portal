-- CRITICAL FIX: Resolve function overloading conflict
-- This maintains the original function name but fixes the parameter type conflict
...
-- Step 2: Create the single correct function with the original name
CREATE FUNCTION upsert_writing_assessment(
  ...
  p_strengths TEXT,  -- Accept JSON strings
  p_improvements TEXT, -- Accept JSON strings
...
RETURNS VOID AS $$
BEGIN
  -- Delete any existing assessment for this question/session
  DELETE FROM writing_assessments
  WHERE user_id = p_user_id 
    AND session_id = p_session_id 
    AND question_id = p_question_id;
  
  -- Insert the new assessment, converting JSON strings to JSONB
  INSERT INTO writing_assessments (
    ...
    p_strengths::jsonb,     -- Convert JSON string to JSONB
    p_improvements::jsonb,  -- Convert JSON string to JSONB
    p_claude_model_version, p_processing_time_ms,
    p_prompt_tokens, p_response_tokens
  );
  
  RAISE LOG 'Writing assessment stored successfully for session % question %', p_session_id, p_question_id;
END;
