...
/**
 * VIC Selective Entry Practice Test 1 Generator
 * 
...
async function main() {
  console.log('🚀 VIC Selective Entry Practice Test 1 Generator');
  ...
  try {
    ...
    if (result.sectionsGenerated && result.sectionsGenerated.length > 0) {
      ...
      result.sectionsGenerated.forEach(section => {
        ...
        if (section.passageIds && section.passageIds.length > 0) {
          console.log(`     └─ ${section.passageIds.length} passages`);
        }
      });
    }
    ...
    console.log('\n🎉 VIC Selective Entry Practice Test 1 is ready!');
