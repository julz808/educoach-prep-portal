...
async function fetchAllNumberOperationsQuestions() {
  console.log('Fetching ALL Number Operations & Properties questions...\n');
  ...
  output += `ALL ${data.length} Questions for Manual Verification\n`;
  output += '=' .repeat(80) + '\n\n';
  ...
  // Save to /tmp/
  const filename = `/tmp/number_operations_all_${data.length}.txt`;
  fs.writeFileSync(filename, output);
  console.log(`✓ Saved to ${filename}`);
  console.log(`\nTotal questions fetched: ${data.length}`);
  console.log('\n⚠️  ERRORS FOUND in first 30 - reviewing ALL questions!');
}

fetchAllNumberOperationsQuestions();
